...
const AddEditIssue: React.FC<AddEditIssueProps> = ({ userRights }) => {
    ...
    useEffect(() => {
        if (issueId && issueId !== "0") {
            apiGetIssueById((response: any) => {
                parseIssueResponse(response.data);
            }, showErrorToast, issueId)
        }
    }, [issueId])

    const parseIssueResponse = (data: any) => {
        setFormData({
    ...
    //Function to fetch deliveries based on search term
    const fetchDeliverySearchResults = async (search: string) => {
        setLoading(true);
        try {
            await apiSearchDeliveryByCourierId((deliveries: Delivery[]) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        if (issueId === '0') {
            apiAddIssue((response: any) => { navigate(-1); showSuccessToast(response.message) }, showErrorToast, formData.delivery_id, formData.description)
        } else {
            apiUpdateIssue((response: any) => { navigate(-1) }, showErrorToast, Number(issueId), formData.delivery_id, formData.description)
        }
    };
    return (
        <>
