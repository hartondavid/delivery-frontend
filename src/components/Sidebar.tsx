...
const Sidebar: React.FC<SidebarProps> = ({ open, onClose, userRights }) => {
    const location = useLocation();
    const [openedCatgory, setOpenedCatgory] = useState<number | null>(null);
    const url = location.pathname;

    const handleCategoryClick = (menuId: number): void => {
            ...
            <Box sx={{ p: 2 }}>
                <List>
                    {url.includes("dashboard") && menus.map(menu => {
                        ...
                        if (shouldShowMenu(userRights, { ...menu, rights: menu.rights.map(String) }))
                            if (menu.isCategory) {
                                    ...
                                    <Collapse in={openedCatgory === menu.id} timeout="auto" unmountOnExit>
                                        <List component="div" disablePadding>
                                            {menu.children.map(child => {
                                                if (shouldShowMenu(userRights, { ...child, rights: child.rights.map(String) }))
